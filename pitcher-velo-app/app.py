...
.dk-bias th:nth-child(1), .dk-bias td:nth-child(1) { width: 120px; white-space: nowrap; }
.dk-bias th:nth-child(2), .dk-bias td:nth-child(2) { width: auto; white-space: nowrap; }
.dk-mix th:nth-child(1), .dk-mix td:nth-child(1) { width: 120px; }
.dk-mix th:nth-child(2), .dk-mix td:nth-child(2) { width: 120px; }
.dk-mix th:nth-child(3), .dk-mix td:nth-child(3) { width: 140px; }
.dk-mix th:nth-child(4), .dk-mix td:nth-child(4) { width: auto; }
</style>
"""
...
def build_bias_tables(df):
    def make(side):
        ...
        for c, g in df[df["stand"] == side].groupby("count"):
            ...
            m = v.mean()
            p = (v >= m).mean()
            rows.append({"Count": c, "Bias": f"{round(max(p,1-p)*100,1)}% {'Over' if p>=.5 else 'Under'} {m:.1f}"})
...
def build_pitch_mix(df):
    if df.empty: return pd.DataFrame(columns=["Pitch", "Usage", "Avg Velo", ""])
    g = df.groupby("pitch_type").agg(P=("pitch_type","size"), V=("release_speed","mean")).reset_index()
    g["Usage"] = (g["P"]/g["P"].sum()*100).round(1).astype(str)+"%"
    g["Avg Velo"] = g["V"].round(1)
    return g.rename(columns={"pitch_type":"Pitch"}).drop(columns=["P","V"]).sort_values("Usage", ascending=False)
...
if not run or away.startswith("—") or home.startswith("—"): st.stop()
...
for t, key in zip(tabs, ["All","Early (1–2)","Middle (3–4)","Late (5+)"]):
    with t:
        for label, df in [("Away Pitcher", away_df), ("Home Pitcher", home_df)]:
            ...
            hand = get_pitcher_throws(df)
            render_pitcher_header(label.split()[0] == "Away" and away or home, f"{hand} | {label} • {key} • {season}")

            mix_sel = st.selectbox("Pitch Mix", ["Overall","vs LHB","vs RHB"], key=f"{label}-{key}")
            mix_df = g if mix_sel=="Overall" else g[g["stand"] == ("L" if mix_sel=="vs LHB" else "R")]
            render_table(build_pitch_mix(mix_df), "dk-mix")

            lhb, rhb = build_bias_tables(g)
            cL, cR = st.columns(2)
